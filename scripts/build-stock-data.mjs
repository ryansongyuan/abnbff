...
function normalizeRows(source) {
  return source.data.tradesTable.rows
    .map((row) => ({
      date: row.date,
...
export function buildStockData(source, yearsToInclude = DEFAULT_YEARS) {
...
if (process.argv[1] && pathToFileURL(process.argv[1]).href === import.meta.url) {
  ...
  fs.writeFileSync(output, JSON.stringify(buildStockData(source)));
}
